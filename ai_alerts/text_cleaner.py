...
def clean_json_response(text: str) -> str:
    """
    Специальная очистка для JSON-ответов от нейросети
    """
...
def safe_json_parse(text: str) -> list:
    """
    Безопасный парсинг JSON, возвращает список новостей
    """
    try:
        data = json.loads(text)

        # Если это словарь, проверяем возможные форматы
        if isinstance(data, dict):
            # Формат с полем "news"
            if "news" in data and isinstance(data["news"], list):
                return data["news"]
            # Формат с полем "articles"
            elif "articles" in data and isinstance(data["articles"], list):
                return data["articles"]
            # Одиночная новость
            elif "text" in data and "tags" in data:
                return [data]
            else:
                return []
